/**
 * Signal processor for BedDot geophone data (web).
 * Ported from backend/src/services/signal-processor.ts — same pipeline as test/visual_vitals.py.
 */
...
/** Respiration: find peaks by prominence (height above neighbors), matching scipy find_peaks. */
function findRespPeaks(
  data: Float64Array,
  minDistance: number,
  minProminence: number
): number[] {
...
/**
 * Process a buffer of raw geophone samples into vital signs (mirrors backend).
 */
